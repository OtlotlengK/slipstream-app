(() => {
  ...
  async function activity(uid) {
    ...
    d.invoices.forEach(x=>items.push({type:'invoice',id:x.id,title:x.invoice_no||'Invoice',subtitle:x.customer_name||x.customer_email||'Customer not provided',status:x.status,amount:x.total,currency:x.currency||'ZAR',date:x.created_at,href:`invoice.html?id=${encodeURIComponent(x.id)}`,search:[x.invoice_no,x.customer_name,x.customer_email,x.total,x.status].join(' ')}));
    payments.forEach(x=>items.push({type:'payment',id:x.id,title:'Payment '+(x.provider_reference||String(x.id).slice(0,8)),subtitle:x.provider||'Payment provider',status:x.status,amount:x.amount,currency:x.currency||'ZAR',date:x.paid_at||x.created_at,href:x.invoice_id?`invoice.html?id=${encodeURIComponent(x.invoice_id)}`:'invoices.html',search:[x.provider,x.provider_reference,x.provider_transaction_id,x.amount,x.status].join(' ')}));
