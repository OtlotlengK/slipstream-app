...
serve(async(req)=>{
 ...
 if(keyError)return json({error:"authentication_error"},500);if(!key||key.revoked_at)return json({error:"invalid_api_key"},401);if(key.environment!=="production")return json({error:"sandbox_not_available"},403);if(!hasPermission(key.permissions,"transactions","events:create"))return json({error:"insufficient_permissions"},403);
 const {data:limitResult,error:limitError}=await db.rpc("check_api_key_rate_limit",{p_api_key_id:key.id,p_limit:60,p_window_seconds:60});if(limitError)return json({error:"rate_limit_check_error"},500);const limit=Array.isArray(limitResult)?limitResult[0]:limitResult;if(!limit?.allowed)return json({error:"rate_limit_exceeded"},429,{"Retry-After":String(Number(limit?.retry_after_seconds)||60)});
 ...
 return json({success:true,event:{id:event.id,receipt_id:event.receipt_id,event_type:event.event_type,created_at:event.created_at},protocol_version:"1.4"},201);
});