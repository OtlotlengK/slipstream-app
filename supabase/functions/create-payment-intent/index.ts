  ...
  try {
    ...
    if (!intent.created) {
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        await sleep(400);
        const { data: latest, error: latestError } = await db.from('payment_intents').select('status,provider_reference,authorization_url,access_code').eq('id', intent.payment_intent_id).maybeSingle();
    ...
    const paystackResponse = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({ email: invoice.customer_email, amount: String(amountSubunit), currency: intent.currency || 'ZAR', reference: intent.provider_reference, callback_url: callbackUrl, channels: ['card', 'bank', 'eft', 'qr', 'capitec_pay', 'apple_pay'], metadata: { invoice_id: invoice.id, invoice_no: invoice.invoice_no, payment_intent_id: intent.payment_intent_id, source: 'valoratap' } }),
    });
    ...
    const { error: updateError } = await db.from('payment_intents').update({ status: 'initialized', authorization_url: result.data.authorization_url, access_code: result.data.access_code || null, updated_at: new Date().toISOString(), metadata: { source: 'valoratap', paystack_access_code: result.data.access_code || null } }).eq('id', intent.payment_intent_id).eq('status', 'pending');
    if (updateError) {
