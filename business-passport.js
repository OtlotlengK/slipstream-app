(() => {
  ...
  async function sharePassport() {
    ...
    try {
      ...
      publicUrl = new URL('business-passport-public.html?token=' + encodeURIComponent(r.public_token), location.href).href;
      $('qr').src = 'https://quickchart.io/qr?size=240&margin=1&text=' + encodeURIComponent(publicUrl);
      $('qr-wrap').classList.remove('hidden');
      $('share-status').textContent = 'Secure Passport link generated. The previous public link is now invalid.';
      ...
      else { try { await navigator.clipboard.writeText(publicUrl); $('share-status').textContent = 'Secure Passport link generated and copied to clipboard.'; } catch (_) { prompt('Copy this secure Passport link:', publicUrl); } }
    } catch (err) {
